...
const Player: React.FC<PlayerProps> = ({ 
  ...
  return (
    <div className="h-auto md:h-40 bg-white/95 backdrop-blur-2xl border-t border-slate-100 flex flex-col shadow-[0_-8px_30px_rgba(0,0,0,0.04)] px-4 md:px-12 pb-safe">
      ...
      <div className="w-full pt-4 md:pt-6">
        <div className="flex items-center gap-3 md:gap-6">
           ...
           <div className="flex-1 relative h-6 flex items-center group">
            ...
            <div 
              className="absolute w-3 h-3 bg-white border-2 border-[#4da8ab] rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"
      ...
      <div className="flex flex-col md:flex-row flex-1 items-center justify-between gap-4 md:gap-0 py-4 md:py-0">
        {/* Track Info Section */}
        <div className="w-full md:w-1/3 flex items-center gap-3 md:gap-4 order-1 md:order-last">
          <img src={track.coverUrl} className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl object-cover shadow-md shrink-0" alt="" />
          <div className="min-w-0 flex-1">
            <h4 className="font-bold truncate text-slate-800 text-sm md:text-base leading-tight">{track.name}</h4>
            <p className="text-[10px] md:text-xs text-[#4da8ab] font-medium opacity-70 truncate">{track.artist}</p>
          </div>
          <button 
            onClick={onToggleFavorite} 
            className={`p-2 transition-colors ${track.isFavorite ? 'text-rose-500' : 'text-slate-300 hover:text-slate-400'}`}
          >
        ...
        <div className="flex items-center justify-center gap-6 md:gap-10 order-2">
          {/* Skip Forward (أيقونة التراجع تتقدم) */}
          <button onClick={() => onSkip(10)} className="text-slate-400 hover:text-[#4da8ab] transition-colors active:scale-90">
          ...
          {/* Skip Backward (أيقونة التقدم تتراجع) */}
          <button onClick={() => onSkip(-10)} className="text-slate-400 hover:text-[#4da8ab] transition-colors active:scale-90">
            <svg className="w-8 h-8 md:w-10 md:h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
        ...
        <div className="w-full md:w-1/3 flex items-center justify-between md:justify-start gap-4 order-3 md:order-first">
          <div className="flex items-center gap-2">
            <button onClick={onAddTimestamp} className="p-2 text-[#4da8ab] hover:bg-[#4da8ab]/5 rounded-lg transition-colors">
            ...
            <select 
              ...
              className="bg-slate-50 border border-slate-100 text-[10px] md:text-xs font-bold text-slate-500 px-3 py-1.5 md:py-2 rounded-lg outline-none cursor-pointer"
            >
