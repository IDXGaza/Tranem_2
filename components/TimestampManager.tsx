  ...
  return (
    <div className="w-full flex flex-col space-y-4">
      <div className="flex items-center justify-between px-2">
        <h3 className="font-black text-slate-800 dark:text-slate-200 text-lg md:text-xl">العلامات الزمنية</h3>
      ...
      <div className="grid grid-cols-1 gap-2.5">
        {timestamps.length === 0 ? (
          <div className="py-12 text-center bg-white/40 dark:bg-slate-900/20 border border-dashed border-slate-200 dark:border-slate-800 rounded-[24px]">
          ...
          [...timestamps].sort((a, b) => a.time - b.time).map((ts, index) => (
            <div 
              key={ts.id} 
              className="flex items-center gap-4 p-4 bg-white/80 dark:bg-slate-900/60 hover:bg-white dark:hover:bg-slate-800 rounded-[24px] border border-white dark:border-slate-800/50 transition-all shadow-sm hover:shadow-md group"
            >
