...
const TimestampManager: React.FC<TimestampManagerProps> = ({ 
  ...
  return (
    <div className="bg-white/40 rounded-[40px] p-10 border border-white shadow-sm backdrop-blur-md">
      <div className="flex items-center justify-between mb-10">
        <h3 className="font-bold text-xl text-slate-800">العلامات المحفوظة</h3>
        <span className="bg-[#4da8ab] text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-sm">
      ...
      <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
        {timestamps.length === 0 ? (
          <div className="py-20 text-center opacity-30">
            ...
            <p className="text-sm">انقر مرتين على شريط الوقت لحفظ لحظة</p>
          </div>
        ) : (
          [...timestamps].sort((a, b) => a.time - b.time).map((ts, index) => (
            <div 
              key={ts.id} 
              className="flex items-center justify-between p-4 bg-white/60 hover:bg-white rounded-3xl border border-transparent hover:border-[#4da8ab]/10 transition-all group shadow-sm hover:shadow-md"
            >
              <button 
              ...
              <button 
                ...
                className="text-2xl font-bold tracking-[0.1em] text-[#4da8ab] hover:scale-105 transition-transform"
                style={{ direction: 'ltr' }}
              >
